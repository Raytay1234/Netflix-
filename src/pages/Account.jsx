...
export default function Account({ setPage, user, setUser, darkMode }) {
    ...
    <div className={`${bg} min-h-screen pt-20 px-6 md:px-12 transition-colors duration-300`}>
      ...
      {user ? (
        <div className={`${cardBg} p-6 rounded-md shadow-lg max-w-3xl mb-8 transition-colors duration-300`}>
          <p className="mb-2"><span className={`${secondaryText}`}>Name:</span> {user.name}</p>
          <p className="mb-2"><span className={`${secondaryText}`}>Email:</span> {user.email}</p>
          <p className="mb-2"><span className={`${secondaryText}`}>Plan:</span> {user.plan || "Basic"}</p>
          <p className="mb-2"><span className={`${secondaryText}`}>Billing:</span> {user.billing || "Next billing: TBD"}</p>
        </div>
      ) : (
